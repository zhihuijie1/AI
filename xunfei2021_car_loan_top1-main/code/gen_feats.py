...
''' 特征生成函数 '''
'''目的是通过引入新的特征或转换现有特征，增强模型的学习能力，提高其在特定任务（如分类或回归）上的性能'''
def gen_new_feats(train, test):
    '''生成新特征：如年利率/分箱等特征'''
    # Step 1: 合并训练集和测试集，目的在两部分数据上应用相同的特征工程，保证数据的一致性
    ...
    for col in amount_cols:
        total_monthly_payment_bin = [-1, 5000, 10000, 30000, 50000, 100000, 300000, 500000, 1000000, 3000000,
                                     data[col].max()]
